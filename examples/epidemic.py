...
def Model(initial_cases, initial_date, N, beds_per_100k, R_0_start, k, x0, R_0_end, p_I_to_C, p_C_to_D, s,
          r0_y_interpolated=None):
    days = 360
            ...
            [
                ...
                dash_table.DataTable(
                    ...
                    data=[
                        ...
                        for i in [("2020-01-01", 3.2), ("2020-02-01", 2.9), ("2020-03-01", 2.5), ("2020-04-01", 0.8),
                                  ("2020-05-01", 1.1), ("2020-06-01", 2.0), ("2020-07-01", 2.1), ("2020-08-01", 2.2),
                                  ("2020-09-01", 2.3)]
                    ],
...
def update_graph(_, initial_cases, initial_date, population, icu_beds, p_I_to_C, p_C_to_D, r0_data, r0_columns):
    ...
    r0_data_y = [datapoint["R value"] if ((not np.isnan(datapoint["R value"])) and (datapoint["R value"] >= 0)) else 0
                 for datapoint in r0_data]
    ...
    dates, S, E, I, C, R, D, R_0_over_time, total_CFR, daily_CFR, B = Model(initial_cases, initial_date, population,
                                                                            icu_beds, 3.0, 0.01, 50, 2.3,
                                                                            float(p_I_to_C) / 100,
                                                                            float(p_C_to_D) / 100, 0.001,
                                                                            r0_y_interpolated)

    return {
               # return graph for compartments, graph for fatality rates, graph for reproduction rate, and graph for deaths over time
               'data': [
               ...
               'data': [
                   {'x': dates, 'y': [0] + [D[i] - D[i - 1] for i in range(1, len(dates))], 'type': 'line',
                    'name': 'total'},
                   {'x': dates, 'y': [0] + [max(0, C[i - 1] - B[i - 1]) for i in range(1, len(dates))], 'type': 'line',
